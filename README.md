...
| `P` `B` | порталы · библиотека |
| `F` | действующие эффекты (в графике они внутри «Героя») |
| `L` | журнал игры целиком (только в графике) |
...
- **Пятьдесят локаций** в шести регионах — весь план лорной карты:
  ...
  Уровни в таблице — из плана лорной карты; кривая опыта подобрана так, что
  всё содержимое игры выводит героя примерно к двадцать седьмому уровню, —
  то есть на семь навыков приходится около двух с половиной десятков баллов,
  и выбирать, куда вложить очередной, приходится до самого конца.
...
- **Настоящая темнота в подземельях.** В пещере, шахтах, колодце, могиле и
  схроне видно не всю карту, а только то, до чего дотягивается свет: без огня
  клетку вокруг себя, с факелом в руке — шесть. Остальное не затемнено, а
  просто не нарисовано. Стены свет не пропускают: за углом темно, как бы
  близко он ни был. Факел занимает ту же руку, что и щит, — под землёй
  приходится выбирать между защитой и зрением.
...
- **Бой со всеми разом:** если героя окружили, дерутся все — один его ход и
  по ходу каждого противника. В бой втягиваются те, кто стоит вплотную (все
  восемь клеток вокруг, а не четыре стороны: враг наискосок стоит рядом), и
  подошедший вступает сам в конце раунда. Больше восьми не набирается — их
  просто некуда поставить. Гибель одного бой не заканчивает: удар переносится
  на следующего. Из кольца труднее вырваться — каждый сверх первого отнимает
  часть шанса на побег; замерено на сорока попытках: от одного уходят в 65%
  случаев, от троих — в 30%.
...
- **Прокачка:** опыт, уровни, 7 навыков, очко навыка за уровень. Предела у
  ранга нет, и **само по себе не растёт ничего** — ни здоровье, ни меткость,
  ни очки действия. Уровень даёт балл, а во что его вложить, решает игрок.
  Прежде к каждому уровню сами прибавлялись +4 здоровья и +1 меткости, а
  каждый третий давал очко действия; «Крепость», «Меткость» и «Дыхание» были
  добавкой к тому, что и так капало, и половина списка навыков теряла смысл.
  Следствие проверяется тестом: герой двадцатого уровня, не потративший ни
  балла, по характеристикам в точности равен герою первого — и так же гибнет
  от лесного вожака.

  Цена победы измерена честными поединками без лечения, по 24 схватки на
  расклад (снаряжение не покупается, поэтому это нижняя граница требований):

  | Противник | Баллов навыка | Побед | Остаётся здоровья |
  | --- | ---: | ---: | ---: |
  | Амбарная крыса | 0 | 100% | 98% |
  | Волк | 0 | 100% | 74% |
  | Разбойник | 0 | 91% | 37% |
  | Тень с погоста | 6 | 75% | 20% |
  | Вожак стаи | 14 | 79% | 17% |

  Требование растёт вместе с содержимым — это и проверяется тестом, а не
  конкретные числа. Меткость героя (62) против самого высокого блока в игре
  (36) даёт 26% попаданий: не вложившийся в «Меткость» дерётся втрое дольше,
  но в пол в 5% не упирается — тупика нет, есть цена.
- **Снаряжение:** 5 слотов, больше 30 предметов. Клик по надетой вещи
  открывает её характеристики с кнопкой «Снять», а не разоблачает героя
  молча: промах по строке посреди боя обходился дорого.
- **Торговля:** 3 магазина со своими наценками, вкладки покупки и продажи.
  Квестовые предметы продать нельзя. Тап по товару открывает витрину:
  описание, цена этой лавки и сравнение с тем, что надето в то же гнездо
  (`броня 2 -> 4 (+2)`), — покупать вслепую не приходится. Удержание на
  строке продажи открывает окно количества с ползунком и полем ввода;
  набранное обрезается прямо в поле до того, сколько есть на руках.
...
Карты можно править руками или редактором
...
Файлы карт первичны: их правят, а скрипт `tools/check_maps.py` их читает и
проверяет — ничего не пишет и ничего не перезаписывает. Он ловит то, чего не
видно глазом на сетке 48×18: строку не той ширины, неизвестный символ, объект
в стене или за границей, отрезанный угол локации, куда игрок не попадёт
никогда, и переход, ведущий в несуществующую локацию или на непроходимую
клетку. После правки карт:
...
`make test` — 3769 проверок в одном бинарнике. Кроме обычных модульных, там:
...
- **темнота и баланс**: под землёй без огня видно клетку, с факелом шесть, за
  стеной ничего; факел и щит спорят за одну руку; сквозь броню всегда проходит
  часть удара, а кривая опыта растёт быстрее прямой — сквозное прохождение
  обязано укладываться в вилку по уровню, иначе игра снова станет неуязвимой;
...
Сверх этих семнадцати у каждого жителя и врага может быть своя картинка —
`npc_elder.png`, `mob_wolf.png`. Заготовки на все девяносто существ заводятся
командой `python3 tools/make_creature_tiles.py`; уже нарисованные файлы она не
трогает. Ступеней получается три, и каждая работает сама по себе: своя
картинка существа, иначе общий тайл его вида, иначе знак шрифта.
...
make test     # 3769 регрессионных проверок
