...
```bash
make gui      # графическая сборка на SDL2
make run      # собрать и сразу запустить
make test     # 3569 регрессионных проверок
...
Игра собирается двумя оболочками поверх одной логики:

```bash
make        # терминальная сборка, без зависимостей   -> ./andors-love
make gui    # графическая на SDL2                     -> ./andors-love-gui
```

Графическая рассчитана на телефон: цветная карта с камерой за героем,
всплывающие полупрозрачные окна, кнопки под палец. **Тап** по полю — идти
к точке; **удержание** — бежать за пальцем; **свайп** — шаг в сторону; тап
по себе или в стену, когда герой стоит, — внутриигровое меню с теми же
пунктами, что клавиши `C I Q K M`, но кнопками с подписями. Тап по пункту
выбирает сразу, тап мимо окна закрывает окно. Клавиатура работает как
раньше и нигде не обязательна — как и Escape.

Подробности, включая почему здесь не ImGui и как снимать экраны без
экрана, — в [`docs/gui.md`](docs/gui.md).

## Что уже есть

- **Пятьдесят локаций** в шести регионах. **Ольховый лоскут:** Ольховка,
  лес, Барсучья пещера, Развалины заставы, Ольховский погост, Гнилая топь,
  Святилище Нулевой точки и скрытый Схрон Ордена. **Шов:** Козья тропа, Стеклянное поле, Рынок Шва, Мельница на
  Тихой, Соляные шахты, Пустой караван-сарай, Хутор Двоеданный и Подвесной
  мост в никуда. **Половины:** Половина Города, Улица без конца, Литейный двор,
  Канал Мёртвой воды, Башня Счетовода и Городской архив. **Орден:** Ворота
  Ордена, Привратная, Библиотека, Чертёжный зал, Кельи послушников, Печь
  Ордена, Зал Отказа, Узел Второй, Узел Третий и Могила Первого Мастера.
  **Дрейф:** Дрейфующий луг, Дом на отшибе, Колодец Двух Вёдер, Роща, где не
  темнеет, Поле после битвы, Вторая Половина Города, Лестница вверх, Край
  Лоскута, Пустая Ольховка и Тропа Возвращения. **Изнанка:** Первый Шов,
  Галерея Линий, Комната Измерений, Встреча, Узел Первый, Сердце Стяжения,
  Точка Ноль и Развязка.
...
src/gfx/         графическая оболочка на SDL2: шрифт, жесты, виджеты, экраны
