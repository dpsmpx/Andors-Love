...
```bash
./andors-love-gui --export-tiles data/tiles/tiles.png   # выгрузить текущий вид
```

Получится лист 8×8 тайлов в формате редактора
[True Joint Tile Maker](https://github.com/dpsmpx/TrueJointTileMaker) — рисуйте
поверх и запускайте игру. Ненарисованные тайлы продолжают выглядеть как
раньше, так что набор можно делать по одному.
...
make test     # 3712 регрессионных проверок
...
ОСТАЛЬНОЕ
data/maps/       карты
data/tiles/      лист тайлов: своя графика карты (docs/tiles.md)
...
`make test` — 3712 проверок в одном бинарнике. Кроме обычных модульных, там:
...
- **PNG и тайлы**: чужой PNG с динамическими кодами Хаффмана читается пиксель
  ...
  причиной; лист тайлов собирается и разрезается по правилам редактора, и ни
  один слот не занят дважды;
