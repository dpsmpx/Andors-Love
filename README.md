...
| `F` `P` | действующие эффекты · порталы |
...
`*` предмет · `&` лежанка · `C` сундук · `O` твой портал · `#` стена ·
`T` дерево · `~` вода · `.,=` земля.
...
- **Пять локаций:** Ольховка (5 NPC, 3 торговца, лежанка), Ольховый лес,
  Барсучья пещера, Развалины заставы и Святилище Нулевой точки. Граф переходов
  двусторонний и проверяется тестом.
- **Восемь квестов**, связанных в цепочку: следующий открывается, только когда
  закрыт предыдущий. Все проходимы — это проверяет тест, который проходит их
  настоящим боем от первого разговора до последней награды.
...
- **Сохранение** в текстовый файл `saves/hero.sav` (формат версии 2).
...
Из дизайн-документа не реализовано: торговля предметами между мобами, ставящиеся
игроком таблички. Из технического:

- **Луч видимости.** Радиус обнаружения считается по прямой и не видит стен,
  поэтому моб замечает героя сквозь дом или заросли.
- **Возврат в зону спавна.** Уползший моб продолжает занимать слот своей зоны
  и блокирует появление нового — зоны пустеют на месте, оставаясь
  «заполненными» по счётчику.
- Несколько ячеек сохранения.
