...
Нужен компилятор с C++17 (g++ 9+ / clang 10+ / MSVC 2019+) и `make`.
...
Сборка идёт с `-Wall -Wextra -Wpedantic -Wshadow` и проходит без единого
предупреждения.

**На смартфоне** (Termux) — см. [`docs/termux.md`](docs/termux.md).
...
Знаки карты: `@` ты · `>` переход · `!` табличка · `*` предмет · `&` лежанка ·
`#` стена · `T` дерево · `~` вода · `.,=` земля. Заглавные буквы — NPC
(`M` Мирон, `L` Лада, `B` Бран, `G` Гурий, `H` отшельник), строчные — враги
...
- **Две локации:** Ольховка (4 NPC, 2 торговца, лежанка, таблички) и Ольховый
  лес (отшельник, волки, разбойники, вожак стаи), связанные переходом.
- **Три квеста:** «Волчья напасть» (убить пятерых), «Амулет Лады» (добыть
  предмет с вожака), «Заказ кузнеца» (принести три шкуры). Все проходимы —
  это проверяется тестом, который проходит их настоящим боем.
...
- **Снаряжение:** 5 слотов (оружие, броня, шлем, щит, кольцо), 14 предметов;
  бонусы складываются в итоговые характеристики.
...
- **Диалоги:** узлы и варианты ответа описаны данными, с условиями по этапу
  квеста, счётчикам и предметам.
- **Мир живёт:** мобы бродят, агрессивные преследуют в радиусе обнаружения,
  зоны спавна пополняются по таймеру респавна.
- **Сохранение** в текстовый файл `saves/hero.sav`.
...
docs/termux.md   как собрать и играть на Android
REVIEW.md        ревью прототипа, с которого всё начиналось
```

## Что дальше

Из дизайн-документа (`docs/design.md`) пока не реализовано: таймер эффектов
и сами эффекты (яд, регенерация), инвентарь мобов, сундуки, специализации и
расы, зачарования, порталы. Из технического — поиск пути для преследования
за пределами зоны спавна и сохранение нескольких персонажей.
