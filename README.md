...
make gui          # графическая сборка, нужен SDL2
./andors-love-gui
```
...
| Жест | Действие |
|---|---|
| **Тап по полю** | идти к точке, пока не упрёшься или не тапнешь ещё раз |
| **Удержание** | бежать: герой ускоряется и идёт за пальцем |
| **Свайп** | шаг в сторону; длинный свайп — несколько шагов подряд |
...
`make test` — 3572 проверки в одном бинарнике. Кроме обычных модульных, там:
...
- **жесты и ходьба**: тап, порог свайпа, удержание, два пальца; ходьба
  доходит, упирается, бежит быстрее и прерывается разговором.
