...
void Game::grant_exp(int amount) {
    ...
    while (plr_.exp >= exp_to_next()) {
        ...
        ++plr_.level;
        ++plr_.skill_points;
        // Уровень даёт ровно одно: балл навыка. Ни здоровье, ни меткость, ни
        // очки действия сами не растут. Прежние надбавки принимали решение за
        // игрока и обесценивали половину списка навыков: «Крепость», «Меткость»
        // и «Дыхание» были добавкой к тому, что и так капало каждый уровень.
        // Теперь герой ровно такой, каким его собрали.
        Stats t = total();
        plr_.hp = t.max_hp;
        plr_.ap = t.max_ap;
