...
// Предела у ранга нет: очки навыка редки (по одному за уровень), и решение
// «вложить ещё раз в то же или начать другое» игрок принимает сам. Потолок
// же просто отбирал этот выбор, когда до него доходило.
struct SkillDef {
    std::string id;
    std::string name;
    std::string desc;
...
// ---------- предмет словами ----------
// Описание вещи — общее на обе оболочки. Терминал и графика писали его
// порознь и уже разошлись в мелочах; держать текст в двух местах значит
// однажды поправить одно и забыть другое.

// Название, вид, проза, цена и все ненулевые бонусы. indent приписывается
// к каждой строке: в терминале блок стоит с отступом, в окне — без.
std::string item_desc(const ItemDef& d, const std::string& indent = "");

// Чем вещь отличается от надетой в то же гнездо. worn — что там сейчас,
// ноль — гнездо пусто. Ради этого витрина и заводилась: цену видно и в
// списке, а выгоднее ли новый нагрудник старого — по названию не понять.
