...
struct Modal {
    ...
    Modal() : kind(GameMenu), scroll(0), selling(false) {}
    explicit Modal(Kind k) : kind(k), scroll(0), selling(false) {}
};
...
private:
    ...
    void collect_rows(Modal& m, std::vector<Row>* rows, std::vector<std::string>* ids) const;
